...
Three things about it were worth the trouble to get right. The order of the step matters more than any parameter: move the particles before the pressure solve rather than after it and every particle falls a little and is shoved back every frame, so the water simmers forever at rest — no amount of damping hides it. It is drawn as metaballs rather than as particles: each one is splatted as a soft blob into a density field and the field is cut at a threshold, so particles near each other merge into one surface while a lone one stays a droplet, and nothing has to decide which is which.

And the density term in the pressure solve has to work in both directions. Correcting only crowded cells is the obvious reading — it is what stops water compressing — but nothing else in the model ever pulls particles together, so thin patches are permanent while dense ones are pushed out, and the water quietly ratchets itself apart. Left one-way, the same water spread over 14% more cells in 3000 steps, its packing fell from 3.9 to 3.4 particles per cell, and holes opened up through the middle of it. Under-filled cells now pull back in too, but only where they are submerged: at a free surface a cell is supposed to be short of particles, and pulling there would suck the surface flat.
